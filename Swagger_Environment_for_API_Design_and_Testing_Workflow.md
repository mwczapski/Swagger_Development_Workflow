...
To enable the cross-container API development and testing workflow, that relies on the presence of an `openapi.yaml` file in the right directory, it is necessary to make sure that both the Host and the two containers used here, have access to the same `openapi.yaml` file.
...
To following image depicts how the three environments share the Host directory and have access to the same `openapi.yaml` file.

![sharing_env](sharing_env.png)
...
#### Create a script to Start Swagger Editor Container with access to shared `/api` directory 
...
#### Create a script to Start Swagger Codegen Container with access to shared `/api` directory 
...
Edit the shared `openapi.yaml` file and watch stub server code generation and stub server restart
...
In the ordinary course of events a developer who uses the Swagger Editor and the Swagger Codegen containers mentioned in this document would employ the following workflow for a project:
...
The Swagger Codegen container, created from the Swagger Codegen Image [[mwczapski / swagger_codegen](https://hub.docker.com/repository/docker/mwczapski/swagger_codegen)](https://hub.docker.com/repository/docker/mwczapski/swagger_codegen), and used in the environment discussed, is configured to generate nodejs stub server code and to re-start the stub server each time code is re-generated.

The image includes JDK 8, which is necessary to run the swagger-codegen, so the container created from the image can generate and run Java stubs.

The image is based on Debian and includes the Bash shell, so the container created from the image can generate and run Bash stubs. 

It will be necessary to make changes to a script within the container to cause it to generate Java or Bash stubs and to run the stubs when code is re-generated. This is possible but out of scope for this discussion. See `/usr/local/bin/docker-entrypoint.sh` and follow the path from there to the script which is run each time the container is started and re-started if you want make changes.

Strictly speaking server and client stubs in any swagger-codegen supported language can be generated by swagger-codegen in the container, but only nodejs, Java and Bash runtime environment is available so subs in these languages are the only ones that can run inside the container
